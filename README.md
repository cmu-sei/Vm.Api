...
[contracts/](./contracts) holds what this API has agreed with its browser clients: the SignalR method and
message names, and a summary of the OpenAPI surface the committed client in `vm.ui` was generated from.
Both are generated by the suite here, and both are asserted by it and by `crucible-tests` against the
clients. If a change renames a hub method, alters how many arguments a broadcast carries, or moves
anything a client can see over HTTP, `contracts/` is regenerated with it - see
